...
- Addressed code review findings: upstream text is again available for video/audio requests, reference label parsing now only matches explicit tokens, ComfyUI media fallback no longer uses loose substring matching, and the browser regression shows image fields as `图片1`/`图片2`.
