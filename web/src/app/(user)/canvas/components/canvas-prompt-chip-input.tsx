...
/** 生成节点 Composer 的 contentEditable 输入框：@ 引用以缩略图 chip 展示，序列化后仍返回引用标签。 */
export function CanvasPromptChipInput({ value, references, onChange, onSubmit, mentionRequestNonce = 0, className, style, placeholder }: Props) {
    ...
    const tokens = useMemo(() => parseTokens(value, activeLabels), [activeLabels, value]);
    ...
    useEffect(() => {
        ...
        setMention({ query: "", rect: caretRect() });
        setActiveIndex(0);
    ...
    return (
        ...
        >
            ...
            {imagePreview ? (
                <Image src={imagePreview} alt="引用图片预览" style={{ display: "none" }} preview={{ visible: true, src: imagePreview, onVisibleChange: (visible) => !visible && setImagePreview(null) }} />
            ) : null}
        </div>
    );
}
...
function ReferencePreview({ reference }: { reference: CanvasResourceReference }) {
    ...
    useEffect(() => {
        ...
        const resolve = kind === "image" ? resolveImageThumbnailUrl : resolveMediaUrl;
...
function createReferenceChip(reference: CanvasResourceReference, theme: (typeof canvasThemes)[keyof typeof canvasThemes], onImagePreview: (url: string) => void) {
    ...
    wrapper.title = reference.title || reference.label;
    if (reference.kind === "image") {
        ...
        const initial = resolvePreviewUrlFromCache("image", reference.storageKey, reference.previewUrl);
        if (initial) image.src = initial;
        wrapper.appendChild(image);
        if (reference.storageKey) {
            void resolveImageThumbnailUrl(reference.storageKey, reference.previewUrl || "")
                .then((url) => {
        ...
        wrapper.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            onImagePreview(image.currentSrc || reference.previewUrl || "");
        });
    } else {
...
function resolvePreviewUrlFromCache(kind: CanvasResourceReference["kind"], storageKey?: string, previewUrl = "") {
    ...
    return (kind === "image" ? peekImageThumbnailUrl(storageKey) : peekCachedMediaUrl(storageKey)) || "";
}

function isImeComposing(event: { nativeEvent?: { isComposing?: boolean; keyCode?: number; which?: number }; keyCode?: number; which?: number }) {
    const nativeEvent = event.nativeEvent;
    return Boolean(nativeEvent?.isComposing || event.keyCode === 229 || event.which === 229 || nativeEvent?.keyCode === 229 || nativeEvent?.which === 229);
}

function isPlainEnterKey(event: KeyboardEvent<HTMLDivElement>) {
