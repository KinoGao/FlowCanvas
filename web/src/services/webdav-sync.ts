...
async function webdavFetch(config: WebdavSyncConfig, path: string, init: RequestInit) {
    ...
    try {
        ...
        if (config.proxyMode === "backend") return await fetch(apiUrl("/api/webdav-proxy"), { method: "POST", headers: proxyHeaders(url, init.method || "GET", headers), body: proxyBody(init), signal: controller.signal });
