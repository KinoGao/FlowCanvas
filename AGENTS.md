...
实现任何功能时，如果**需求、目标行为、验收条件或实现方式**不清楚，必须按以下顺序处理：

1. **先阅读现有代码、项目文档与外部契约**，从中寻找可直接推导的答案。包括：本项目源码、`AGENTS.md` / `CHANGELOG.md` / `README.md` 等文档、以及第三方供应商公开契约（API 文档 / 数据模型 / 接口路径）。
2. **仍无法确定时，必须停下来向用户确认**，不得继续实现。

**禁止猜测**以下内容，除非能从现有代码、项目文档、用户明确说明或供应商公开契约中**直接推导**得出：
...
- 数据模型、类型、字段结构
- 参数、默认值、边界值
- 第三方接口路径、请求/响应格式、鉴权方式
- 是否要移植某个功能、或某个功能属于哪个范围

**允许直接实现**的，仅限能从上述四种来源（现有代码 / 项目文档 / 用户明确说明 / 供应商公开契约）**直接推导**出的行为。凡需要"猜"才能落地的部分，一律先确认再动手。
...
| 多模型集成（Seedance 2.0、可灵 3.0、Wan 2.6、Seedream 5.0 等） | ✅ 后台全局模型注册中心 + 后端代理；按已注册通用或专用协议调用 |
| 工作流打组 → 保存为模板 → 工具箱复用 | 🟡 已实现待测试（工具箱保存/插入节点组+连线，模板经 `/api/canvas-templates` 按账号持久化） |
...
- 所有正式连线和临时拉线统一使用连续冷银灰底轨，不得把整条线做成青色虚线；正式连线在鼠标悬停或选中节点显示关联关系时叠加蓝色流光，普通状态与仅选中连线不播放流光，临时拉线始终显示流光。拖线落空后显示的创建菜单必须支持点击菜单外或按 Esc 取消，并同步清除悬空线，不得持续等待用户手动关闭。
- 节点连线必须从节点外侧的圆形「+」端口球心发出；端口无需先选中节点，鼠标进入有限半径即显示并磁吸跟随，可直接开始拉线，离开吸附半径后自动回位；磁吸跟随只允许向节点外侧移动（最多贴到节点边缘），不得吸入节点内部；拉线过程中只显示当前吸附目标节点的端口圆球，不再把所有节点的端口全部点亮。
- 节点选中态只显示四个角的缩放控制点；边中点不显示，可保留不可见的边缘缩放命中区。
...
| 5 | 工作流打组 → 保存为模板 → 工具箱复用 | 创建工作流 | 🟡 已实现待测试（工具箱保存/插入节点组+连线，模板经 `/api/canvas-templates` 按账号持久化，跨画布/跨设备可用） |
| 6 | 时间轴视频合成（多片段拼接 + 音视频混合 + 快捷键） | 视频合成 | 🟡 已实现待测试（视频合成节点「时间轴编辑」弹窗：片段拖拽排序、入点/出点裁剪、canvas + Web Audio 混音 + MediaRecorder 合成导出） |
...
| 16 | 视频主体库（多图 / 视频建可复用主体，跨镜头角色一致性） | 主体库 | 🟡 已实现待测试（视频节点 Composer「主体库」：账号级主体 = 名称 + 描述 + 多参考图，生成时参考图并入管线、描述追加提示词） |
| 17 | 运镜控制（20+ 预设运镜：推 / 拉 / 摇 / 俯仰 / 环绕 / 跟随等） | 运镜控制 | 🟡 已实现待测试（视频节点 Composer「运镜库」24 项预设，生成时追加运镜提示词） |
| 18 | 视频工具集：视频高清放大 + 补帧、视频解析（拆成分镜表）、视频剪辑快捷键 | 视频工具 | 🟡 部分实现待测试（视频解析：抽帧 + 识图模型生成分镜表脚本节点；视频剪辑：入点/出点导出片段新节点；视频高清放大+补帧因模型注册中心无视频编辑/放大类能力未实现） |
| 19 | 时间轴完整交互：独立音频轨（BGM / 配音、静音开关）、I/O 出入点、方向键精剪 | 视频合成 | 🟡 已实现待测试（与第 6 项合并：独立音频轨逐片段静音混音、I/O 出入点、←/→ 移动播放头、↑/↓ 0.1s 与 Shift+↑/↓ 0.01s 精调、空格播放、Delete 删片段） |
| 20 | 音频生成（TTS 文本转语音 / 文本生成音乐） | 音频模型 | 🟡 TTS 已实现待测试（音频节点生成全链路：OpenAI 兼容 `/audio/speech` 代理、音色/语速/格式参数与能力校验）；文本生成音乐待注册中心出现可用协议后接入 |
| — | 画布分享链接（只读 / 可编辑，无角色体系） | 分享 | ⏳ 待办（见商业化成熟度清单 P1；实时协同以版本历史替代，已决策不做） |
| 21 | 一键成片编排（创意一句话 → 剧本分镜 → 逐镜生成 → 自动合成导出） | Agent 一站式创作（LibTV/TapNow/UpDream 共性能力） | 🟡 已实现待测试（Agent Run 规划器新增 compose 成片产物：编译为智能剪辑节点并按依赖连线片段，服务端只跑生成任务，前端在全部片段完成后自动下发 video_compose 合成出片；对话空态新增「创意成片」建议卡直达任务规划模式） |
| — | 团队实时协作、统一团队资产库 | 团队版 | ❌ 单机定位不做 |
| — | 积分、付费模型、社区、发布审核 | 商业化 | ❌ v1 不做（TapTV 发布社区同样不纳入） |
...
- 项目不内置火山方舟 Agent Plan 调用协议；模型只能使用后台当前已注册的通用或专用协议，文档不得把已移除协议写成可用能力。
- **未经管理员明确同意，不得执行任何会消耗真实金钱或平台积分/余额的操作**：使用真实 API Key 发起付费模型生成请求、创建付费任务（如图片 / 视频生成）、调用按量计费接口、充值购买等。确需验证或演示时，必须先向管理员列出具体动作、目标厂商与预估消耗（单价 × 数量），得到明确同意后再执行；仅只读查询（任务状态、模型列表、余额等不产生费用）不受此限。
- 当用户要求浏览器调试、查看页面状态、检查页面元素或复现前端问题时，优先使用 `mcp_chrome_devtools_*` 工具（页面导航、截图、DOM 快照检查、控制台与网络请求查看），而不是仅凭代码推断；仅在 DevTools 不可用时才退回到代码阅读与推理。
- **非画布页面 UI 对齐 VOZEB-PRO**（用户已确认方向）：工作台外壳为左侧分组侧边栏（`navigationGroups`：项目/资产）+ 当前页顶栏（`AppShell`），页面用「小字标签 + 大标题 + 描述 + 右侧操作 + 卡片网格」的 VOZEB 页头模式；画布编辑器（`/canvas/:id`）不经过外壳、UI 保持现有形态不动。新增非画布页面时沿用 `AppShell` 与页头模式，不要再加回顶部横向导航。
- 独立的生图工作台 / 视频创作台 / ComfyUI 页面已移除（用户决策：这些能力由画布节点承载，不单列页面）；不要再把它们加回侧边栏或新增平行入口，相关设置面板组件（image/video/audio-settings-panel、model-picker 等）仍被画布 Composer 复用，删除页面不等于删除这些共享组件。
