...
- **非画布页面 UI 对齐 VOZEB-PRO**（用户已确认方向）：工作台外壳为左侧分组侧边栏（`navigationGroups`：创作/项目/资产）+ 当前页顶栏（`AppShell`），页面用「小字标签 + 大标题 + 描述 + 右侧操作 + 卡片网格」的 VOZEB 页头模式；画布编辑器（`/canvas/:id`）不经过外壳、UI 保持现有形态不动。新增非画布页面时沿用 `AppShell` 与页头模式，不要再加回顶部横向导航。
