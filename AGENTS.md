...
| 项目 / 画布 / 素材云同步 | ⏳ 当前只在浏览器本地，云同步不在 v1 范围 |
| 跨画布复制（带连线） | ⏳ v1 待办 |
...
| 持久化 | `localforage`（业务数据） / `localStorage`（极小配置） | 1 |
...
| 3 | 脚本节点 + 分镜表 | 脚本节点 | 🚧 优先 |
| 4 | `/` 快捷功能（九宫格、剧情推演、镜头聚焦、焦点编辑、四宫格、25 宫格连贯分镜等） | 图像 / 视频生成器快捷 | 🚧 优先 |
| 5 | 工作流打组 → 保存为模板 → 工具箱复用 | 创建工作流 | ⏳ |
...
- 当某个渠道在配置里勾选了"后端代理"（用于绕过第三方 API 的 CORS），AI 请求会经后端 `/api/ai-proxy` 走一次服务端透传；此时 API Key 会过服务端但不会落盘，文档里要把这种模式和默认的前台直连区分开。
